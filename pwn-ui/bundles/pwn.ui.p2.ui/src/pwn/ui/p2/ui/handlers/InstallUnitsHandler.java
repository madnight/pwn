...
public class InstallUnitsHandler {
  ...
  URI location = URI.create(
      "http://localhost:8081/nexus/service/local/repositories/master.group.unzip/content/pwn/ui/pwn.ui.update/1.0.0-SNAPSHOT/pwn.ui.update-1.0.0-SNAPSHOT.zip-unzip");
  ...
  private void installNewUnits(IProvisioningAgent agent, IProgressMonitor monitor)
    ...
    IQueryResult<IInstallableUnit> queryResult =
        repository.query(QueryUtil.createLatestIUQuery(), subMonitor);
    checkForInstall(agent, subMonitor.split(1), queryResult);
  }

  private IStatus checkForInstall(final IProvisioningAgent agent, IProgressMonitor monitor,
      IQueryResult<IInstallableUnit> toInstall) {
