...
public class Rcc8 {
  ...
  public boolean disconnected(Square objectA, Square objectB) {
    final double aX = objectA.getPosition().getX();
    final double aY = objectA.getPosition().getY();
    final double bX = objectB.getPosition().getX();
    final double bY = objectB.getPosition().getY();

    final boolean cond1 = objectA.getLength() > 0 && objectA.getLength() > 0
        && objectB.getLength() > 0 && objectB.getLength() > 0;

    final boolean cond2a = aX + objectA.getLength() < bX;
    final boolean cond2b = aY + objectA.getLength() < bY;
    final boolean cond2c = bX + objectB.getLength() < aX;
    final boolean cond2d = bY + objectB.getLength() < aY;
